...
if ( wpppfm_prerequisites() ) {
	...
	if ( function_exists( 'wppfm_show_wp_error' ) ) {
		wppfm_show_wp_error( __( 'You need at least version 2.39.0 of the WP Product Feed Manager plugin to use the WooCommerce Google Review Feed Manager add-on', 'wp-product-review-feed-manager' ) );
	}
}
...
function wpppfm_prerequisites(): bool {
	if ( ( WPPFM_EDD_SL_ITEM_NAME === 'Woocommerce Product Feed Manager'
		|| WPPFM_EDD_SL_ITEM_NAME === 'Woocommerce Google Feed Manager' )
		&& version_compare( WPPFM_VERSION_NUM, '3.0.0', '>=' ) ) {
		return true;
	} elseif ( WPPFM_EDD_SL_ITEM_NAME === 'WP Product Feed Manager'
