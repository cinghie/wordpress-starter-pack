...
function wpppfm_merchantPromotionsFeedSelected() {
	var merchantPromotionsFileNameElement = jQuery( '#wppfm-feed-file-name' );

	// verify if the filename is valid
	if ( false === wppfm_validateFileName( merchantPromotionsFileNameElement.val() ) ) {
		merchantPromotionsFileNameElement.val( '' );
	}

	// now clear the product feed form and place the correct Review Feed elements
	wpppfm_initializeMerchantPromotionsFeedForm( merchantPromotionsFileNameElement.val() );
}
