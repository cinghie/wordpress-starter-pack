...
function wpppfm_initiatePromotionsFeedHolder( feedName ) {
	var feedDataArrayTextElement = jQuery('#wppfm-ajax-feed-data-array').text();

	if ( ! feedDataArrayTextElement ) { return; }

	try {
		var feedData = JSON.parse(feedDataArrayTextElement); // fetch the data from the edit feed form code.
	} catch(e) {
		wppfm_handleJsonParseError(e, feedDataArrayTextElement);
	}
	...
	feedData['url'] = jQuery( '#wp-plugin-url' ).text() + '/wppfm-feeds/' + feedData['feed_file_name'] + '.xml';
...
jQuery(function() {
	...
	if ( 'google-merchant-promotions-feed' !== tabId ) { return; }
