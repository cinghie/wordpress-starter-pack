...
if ( ! class_exists( 'WPPFM_Database' ) ) :
	...
	class WPPFM_Database {
		...
		private $_version = '1.4.0'; // as of plugin version 2.3.0
		private $_wpdb;
		...
		private function update_status_table() {
			...
			if ( $count <= 7 ) {
				...
				$this->_wpdb->replace(
					$table_name,
					array( 'status_id' => '2', 'status' => 'on_hold', 'color' => '#0173AA' ),
					$format
				);
				...
				$this->_wpdb->replace(
					$table_name,
					array( 'status_id' => '4', 'status' => 'in_processing_queue', 'color' => '#00CCFF' ),
					$format
				);
				
				$this->_wpdb->replace(
					$table_name,
					array( 'status_id' => '5', 'status' => 'has_errors', 'color' => '#FF0000' ),
					$format
				);
				
				$this->_wpdb->replace(
					$table_name,
					array( 'status_id' => '6', 'status' => 'failed_processing', 'color'	=> '#FF3300' ),
					$format
				);
			}
		}
