	...
	class WPPFM_Queries {
		...
		public function clean_channel_table() {
			return $this->_wpdb->query( 
				$this->_wpdb->prepare( "DELETE FROM {$this->_table_prefix}feedmanager_channel WHERE channel_id = %d OR name = %s",
				0,
				'') );
		}
		...
		public function read_meta_data( $product_id, $parent_product_id, $record_ids, $meta_columns ) {
			...
			foreach ( $meta_columns as $column ) {
				...
				if ( $taxonomy ) {
					$taxonomy_value = WPPFM_Taxonomies_Class::make_shop_taxonomies_string( $product_id, $taxonomy->name, ', ' );

					if( ! $taxonomy_value ) {
						$taxonomy_value = WPPFM_Taxonomies_Class::make_shop_taxonomies_string( $parent_product_id, $taxonomy->name, ', ' );
					}
