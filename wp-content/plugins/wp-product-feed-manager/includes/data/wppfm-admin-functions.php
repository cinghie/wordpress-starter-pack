 ...
 */
function wppfm_convert_price_date_to_feed_format( $datestamp ) {
	if ( $datestamp ) {
		// register the date
 ...
 * @since 1.4.1
 */
function wppfm_decrease_updatable_channels() {
...
function wppfm_prep_money_values( $money_value, $feed_language = '' ) {
	...
	if( !is_float( $money_value ) ) {
		$val = wppfm_numberformat_parse( $money_value );
 ...
 * @return string
 */
function wppfm_numberformat_parse( $number_string ) {
...
/**
 * For backward compatibility, the old feed statusses are converted to all lowercase and without spaces
 * 
 * @since 2.1.0
 * @param array $list
 */
