	...
	class WPPFM_Ajax_Data_Class extends WPPFM_Ajax_Calls {
		...
		public function myajax_get_list_of_feeds() {
			if ( $this->safe_ajax_call( filter_input( INPUT_POST, 'postFeedsListNonce' ), 'myajax-post-feeds-list-nonce' ) ) {
				...
				// @since 2.1.0 due to implementation of i18n to the plugin and for backwords compatibility we need to change
				...
				switch ( $source_id ) {
					...
					default:
						if ( 'valid' === get_option( 'wppfm_lic_status' ) ) { // error message for paid versions
							echo '<div id="error">' . __( 'Could not add custom fields because I could not identify the channel. 
									If not already done add the correct channel in the Manage Channels page. 
									Also try to deactivate and then activate the plugin.', 'wp-product-feed-manager' ) . '</div>';
							...
							echo '<div id="error">' . __( 'Could not identify the channel. 
								Try to deactivate and then activate the plugin. 
								If that does not work remove the plugin through the Wordpress Plugins page and than reinstall and activate it again.', 'wp-product-feed-manager' ) . '</div>';
