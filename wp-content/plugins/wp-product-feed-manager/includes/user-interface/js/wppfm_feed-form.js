...
function wppfm_validateFileName() {
	if ( ! wppfm_contains_special_characters( jQuery( '#file-name' ).val() ) ) {
...
//	var source = parseInt( jQuery( '#sources' ).val() ); // change back to this readout when the plugin is also capable of running other sources besides WooCommerce
	var source = '1';
	...
	var daysInterval = jQuery( '#days-interval' ).val() !== '' ? jQuery( '#days-interval' ).val() : '1';
	var hours = jQuery( 'update-schedule-hours' ).val() !== '' ? jQuery( 'update-schedule-hours' ).val() : '00';
	var minutes = jQuery( 'update-schedule-minutes' ).val() !== '' ? jQuery( 'update-schedule-minutes' ).val() : '00';
	var frequency = jQuery( 'update-schedule-frequency' ).val() !== '' ? jQuery( 'update-schedule-frequency' ).val() : '1';
	var feedFilter = [ ];
	var status = 2;
...
function wppfm_finishOrUpdateFeedPage( categoryChanged ) {
	...
	var channel = jQuery( '#merchants' ).val().toString();
	
	// make sure the data is correct
	_feedHolder['title'] = jQuery( '#file-name' ).val();
	_feedHolder['channel'] = jQuery( '#merchants' ).val();
	...
	wppfm_outputFields( -1, channel, function ( outputs ) {
		...
		wppfm_customSourceFields( _feedHolder['dataSource'], function ( customFields ) {
			...
			wppfm_mainFeedFilters( _feedHolder['feedId'], function ( feedFilters ) {
				...
				if ( categoryChanged ) {
					_feedHolder['mainCategory'] = jQuery( '#lvl_0' ).val() ? jQuery( '#lvl_0' ).val() : jQuery( '#free-category-text-input' ).val();
					_feedHolder.setMainCategory( 'lvl_0', jQuery( '#lvl_0' ).val(), channel );
				}
...
function wppfm_saveUpdateSchedule() {
	...
	var mins = jQuery( '#update-schedule-minutes' ).val();
...
function wppfm_setScheduleSelector( days, freq ) {
	// change the form selector if required
	if ( days === '1' ) {
		jQuery( '#wppfm-update-frequency-wrapper' ).show();
	} else {
		jQuery( '#wppfm-update-frequency-wrapper' ).hide();
	}
	
	if ( freq > 1 ) {
		jQuery( '#wppfm-update-day-wrapper' ).hide();
		jQuery( '#wppfm-update-every-day-wrapper' ).show();
	} else {
		jQuery( '#wppfm-update-day-wrapper' ).show();
		jQuery( '#wppfm-update-every-day-wrapper' ).hide();
	}
}
...
function wppfm_alert_update_finished( feedId, repeatTime ) {
	if ( undefined === repeatTime || 0 === repeatTime ) { repeatTime = 5000; } // default value
	...
	function wppfm_checkAndSetStatus( feedId ) {
		wppfm_getCurrentFeedStatus( feedId, function( result ) {
			...
			switch( status['status_id'] ) {
				...
				case '1': // on hold
				case '2': // active
					wppfm_show_success_message( wppfm_feed_settings_form_vars.feed_status_ready.replace( '%feedname%', status["title"]).replace( '%prodnr%', status["products"] ) );
					window.clearInterval( wppfmStatusCheck );
					break;
...
function wppfm_saveFeed() {
	...
	if ( newFeed ) _feedHolder['url'] = wppfm_feed_settings_form_vars.no_feed_generated;
...
function wppfm_nextCategory( currentLevelId ) {
	...
	if ( nextLevel > 1 ) { wppfm_show_feed_spinner(); }
	...
	wppfm_getNextCategories( channel, nextLevel, selectedCategory, language, function ( categories ) {
		...
		if ( list.length > 0 ) {
			jQuery( '#' + nextLevelId ).append( wppfm_categorySelectCntrl( list ) );
			jQuery( '#' + nextLevelId ).show();
		}
		...
		if ( nextLevel > 1 ) { wppfm_hide_feed_spinner(); }
	} );
}
...
function wppfm_getCategoryChildren( parentCategoryId ) {
	return jQuery( '#feed-selector-' + parentCategoryId ).attr( "data-children" ) ? JSON.parse( jQuery( '#feed-selector-' + parentCategoryId ).attr( "data-children" ) ) : [ ];
}
...
function wppfm_setChildrenToParentCategory( parentId, childId, level, selectedCategory ) {
	...
	if ( currentChildCategory === preSelectionParentCategory || currentChildCategory === wppfm_feed_settings_form_vars.map_to_default_category ) {
		...
		var children = jQuery( '#feed-selector-' + childId ).attr( "data-children" ) ? JSON.parse( jQuery( '#feed-selector-' + childId ).attr( "data-children" ) ) : [ ];

		for ( var i = 0; i < children.length; i++ ) {
			wppfm_setChildrenToParentCategory( childId, children[i], level, selectedCategory );
		}
...
function wppfm_getCategorySelectorValue( selectorId ) {
	...
	if ( val === '' ) {
		...
		for ( var i = 0; i < 7; i++ ) {
			var cat = jQuery( '#catmap-' + selectorId + '_' + i ).val();
			
			if ( cat !== null && cat !== '0' )
				catString += jQuery( '#catmap-' + selectorId + '_' + i ).val() + ' > ';
		}
...
function wppfm_fillFeedFields( isNew, categoryChanged ) {
	...
	jQuery( '#merchants' ).val( _feedHolder['channel'] );
	jQuery( '#variations' ).prop( 'checked', _feedHolder['includeVariations'] > 0 ? true : false );
	jQuery( '#aggregator' ).prop( 'checked', _feedHolder['isAggregator'] > 0 ? true : false );
 ...
 * @access	public
 * 
 * @param {array}		array with strings of categories to be placed in the selector
 * 
...
function wppfm_childrensValue( parentId ) {
	var childrenIds = wppfm_getCategoryChildren( parentId );
	var childsValue = '';
	var childrenWithoutOwnCat = [];
	var result = false;
	
	for( var i = 0; i < childrenIds.length; i++ ) {
		childsValue = wppfm_getCategorySelectorValue( childrenIds[i] );
		if( childsValue !== '0' && childsValue !== wppfm_feed_settings_form_vars.map_to_default_category ) { 
			result = childsValue;
			jQuery( '#feed-category-' + childrenIds[i] ).html( wppfm_mapToDefaultCategoryElement( childrenIds[i], result ) );
			jQuery( '#category-selector-catmap-' + childrenIds[i] ).hide();
		} else {
			childrenWithoutOwnCat.push(childrenIds[i]);
		}
	}
	
	if ( ! result ) return result;
	
	var level = ( result.match(/ > /g) || [] ).length;
	
	for ( var c = 0; c < childrenWithoutOwnCat.length; c++ ) {
		wppfm_setChildrenToParentCategory( parentId, childrenWithoutOwnCat[c], level, result );
	}
...
function wppfm_showCategoryMappingSelectors( id, catList, currentCategoryArray ) {
	for( var i = 0; i < catList.length; i++ ) {
		if ( catList[i] && catList[i].length > 0 ) {
			...
			jQuery( '#catmap-' + id + '_' + i ).append( wppfm_categorySelectCntrl( catList[i] ) );
			jQuery( '#catmap-' + id + '_' + i ).val( selectedCategory ); 
			jQuery( '#catmap-' + id + '_' + i ).show();
		} 

		jQuery( '#catmap-' + id + '_' + i ).prop( 'disabled', false );
	}
...
function wppfm_activateOptionalFieldRow( level, name ) {
	...
	if ( level === 3 ) {
		jQuery( '#new-recommended-row' ).append( code );
		jQuery( '#new-recommended-row' ).show();
	} else if ( level === 4 ) {
...
function wppfm_deactivateOptionalFieldRow( rowId, level, name ) {
	...
	jQuery( "#output-field-cntrl-" + level ).append( "<option value='" + name + "'>" + name + "</option>" );

	jQuery( '#row-' + rowId ).html( '' );

	jQuery( '#new-recommended-row' ).hide();
...
function wppfm_activateCustomFieldRow( fieldName ) {
	if ( ! _feedHolder.checkIfCustomNameExists( fieldName ) ) { // prevent doubles
		...
		jQuery( '#new-custom-row' ).append( wppfm_fieldRow( _feedHolder['attributes'][attributeId], true ) );
		jQuery( '#new-custom-row' ).show();
	} else {
...
function wppfm_addSourceDataAndQueriesColumn( sourceLevel, sourceRowsData ) {
	...
	} else if ( ( sourceRowsData.advisedSource && !sourceMappingHasSourceData && sourceRowsData.advisedSource !== wppfm_feed_settings_form_vars.fill_with_static_value )
...
function wppfm_addCombinedField( id, sourceLevel, combinedLevel ) {
	...
	for ( var i = 0; i < combinedLevel; i++ ) {
		...
		if ( jQuery( '#combined-input-field-cntrl-' + id + '-' + sourceLevel + '-' + combinedLevel ).val() === 'select' ) {

			allFilled = false;
			i = combinedLevel;
		}

		if ( jQuery( '#combined-input-field-cntrl-' + id + '-' + sourceLevel + '-' + combinedLevel ).val() === 'static'
...
function wppfm_ifConditionSelector( rowId, sourceLevel, conditionLevel, numberOfQueries, queryObject ) {
	...
	htmlCode += '" onclick="wppfm_removeCondition(' + rowId + ', ' + sourceLevel + ', ' + ( conditionLevel - 1 ) + ')">' + wppfm_feed_settings_form_vars.remove + '</a>)';
...
//        jQuery( '#dummy-source-' + rowId + '-' + sourceLevel + '-' + t ).remove();
	...
	if ( nrQueries > 0 ) {
		...
		jQuery( '#source-' + rowId + '-' + ( sourceLevel + 1 ) ).remove();
	}
}
...
function wppfm_removeValueQuery( rowId, sourceLevel, queryLevel ) {
	...
	jQuery( '#edit-conditions-' + rowId ).empty();
	...
	if ( nrQueries > 0 ) {
		...
		jQuery( '#edit-value-span-' + rowId + '-' + ( sourceLevel + 1 ) + '-0' ).remove();
	}
}
...
function wppfm_removeRow( rowId, fieldName ) {
	...
	jQuery( "#output-field-cntrl-" + level ).empty();
	jQuery( "#output-field-cntrl-" + level ).html( wppfm_outputFieldCntrl( level ) );
}
...
function wppfm_showEditValueQuery( rowId, sourceLevel, queryLevel, query ) {
	...
	jQuery( '#value-editor-queries-' + rowId + '-' + sourceLevel + '-' + queryLevel ).html( wppfm_ifValueQuerySelector( rowId, sourceLevel, queryLevel + 1, emptyQueryObject, query ) );
...
function wppfm_changedOutput( rowId, sourceLevel, advisedSource ) {
	...
	switch ( selectedValue ) {
		...
		case 'static':
			...
			jQuery( '#source-select-' + rowId + '-' + sourceLevel ).append( wppfm_displayCorrectStaticField( rowId, sourceLevel, 0, _feedHolder['channel'], _feedHolder['attributes'][rowId]['fieldName'], '' ) );
			jQuery( '#combined-wrapper-' + rowId + '-' + sourceLevel ).remove();

			var staticValue = jQuery( '#static-input-field-' + rowId + '-' + sourceLevel + '-' + combinationLevel ).val() ? jQuery( '#static-input-field-' + rowId + '-' + sourceLevel + '-' + combinationLevel ).val() :
		...
		case 'select':
			...
			jQuery( '#static-input-field-' + rowId + '-' + sourceLevel + '-' + combinationLevel ).remove();
			jQuery( '#static-condition-input-' + rowId + '-' + sourceLevel + '-' + combinationLevel ).remove();
			jQuery( '#combined-wrapper-' + rowId + '-' + sourceLevel ).remove();
...
//            _feedHolder.setCategoryValue( rowId, _feedHolder.mainCategory );    // nieuwe feed maken, aantal outputs invoeren, een van de outputs aan de category map koppelen
			...
			jQuery( '#source-select-' + rowId + '-' + sourceLevel ).html( wppfm_categorySource() );
	...
	switch ( selectedValue ) {
		case 'static':
			if ( ! jQuery( '#static-input-field-' + rowId + '-' + sourceLevel + '-' + combinationLevel ).val() ) {
				...
				jQuery( '#static-value-control-' + rowId + '-' + sourceLevel + '-' + combinationLevel ).html( htmlCode );
			}

			break;

		default:
			jQuery( '#static-value-control-' + rowId + '-' + sourceLevel + '-' + combinationLevel ).empty();
...
function wppfm_storeCondition( rowId, sourceLevel, conditionLevel ) {
	...
	if ( wppfm_validSourceSelected( rowId, sourceLevel ) ) {
		...
		if ( source !== 'select' ) {
			...
			var betweenValue = condition === '14' ? jQuery( '#condition-and-value-input-' + rowId + '-' + sourceLevel + '-' + conditionLevel ).val() : '';
...
function wppfm_validSourceSelected( rowId, sourceLevel ) {
	...
	switch ( mainSelection ) {
		...
		case 'static':
			...
			if ( jQuery( '#static-condition-input-' + rowId + '-' + sourceLevel + '-0' ).val() ) {
				return jQuery( '#static-condition-input-' + rowId + '-' + sourceLevel + '-0' ).val() ? true : false;
			} else if ( jQuery( '#static-input-field-' + rowId + '-' + sourceLevel + '-0' ).val() ) {
				return jQuery( '#static-input-field-' + rowId + '-' + sourceLevel + '-0' ).val() ? true : false;
			} else {
				return false;
			}

		case 'combined':
			return jQuery( '#combined-input-field-cntrl-' + rowId + '-' + sourceLevel + '-2' ).val() !== 'select' ? true : false;
...
function wppfm_queryConditionChanged( id, sourceLevel, conditionLevel ) {
	...
	if ( conditionStored && !jQuery( '#source-' + id + '-' + ( sourceLevel + 1 ) ).length ) {
