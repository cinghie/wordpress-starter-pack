...
function wppfm_showChannelInputs( channel, isNew ) {
	...
	if ( ( null === jQuery( '#lvl_0' ).val() && '' === jQuery( '#selected-categories' ).html() ) || 0 === jQuery( '#countries' ).val() ) {
	...
	switch ( channel ) {
		...
		case '24': // Heureka
			if ( "ITEM_TYPE" === outputsField['field_label'] ) {
				if ( !outputsField['value'] ) { outputsField['value'] = setHeurekaPresets( outputsField['field_label'] ); }
			}
			break;

		case '25': // Pepperjam
			if ( "ITEM_TYPE" === outputsField['discontinued'] ) {
				if ( !outputsField['value'] ) { outputsField['value'] = setPepperjamPresets( outputsField['field_label'] ); }
			}
			break;
