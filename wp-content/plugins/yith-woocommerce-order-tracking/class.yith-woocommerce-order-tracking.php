...
if ( ! class_exists( 'YITH_WooCommerce_Order_Tracking' ) ) {
	...
	class YITH_WooCommerce_Order_Tracking {
		...
		/**
		 * YITH WooCommerce Order Tracking panel page
		 *
		 * @var string
		 */
		...
		public function enqueue_scripts() {
			...
			wp_register_script( 'tooltipster', YITH_YWOT_URL . 'assets/js/jquery.tooltipster.min.js', array( 'jquery' ), YITH_YWOT_VERSION, true );
			wp_enqueue_script( 'tooltipster' );

			wp_enqueue_style( 'ywot_style', YITH_YWOT_URL . 'assets/css/ywot_style.css', array(), YITH_YWOT_VERSION );
		...
		public function show_picked_up_icon( $data, $css_class = '' ) {
			...
			echo '<a class="track-button ' . esc_attr( $css_class ) . ' " style="display:inline-block;height:25px; padding-top:0; padding-bottom:0; top: 10px; position: relative; margin-left:10px" href="#" data-title="' . esc_attr( $message ) . '"><img class="track-button" style="height:25px;" src="' . esc_url( YITH_YWOT_ASSETS_URL ) . '/images/order-picked-up.png" data-title="' . esc_attr( $message ) . '" /></a>';
		}
		...
		public function add_order_shipping_details( $order ) {
			...
			echo '<div class="' . esc_attr( $container_class ) . '">' . wp_kses_post( $this->show_tracking_information( $order, get_option( 'ywot_order_tracking_text' ), '' ) ) . '</div>';
		}
		...
		public function premium_tab() {
			...
			$premium_tab_template = YITH_YWOT_TEMPLATE_PATH . '/admin/' . $this->premium;
		...
		public function show_order_tracking_metabox( $post ) {
			...
			<div class="track-information">
				<p>
					<label
						for="ywot_tracking_code"> <?php esc_html_e( 'Tracking code:', 'yith-woocommerce-order-tracking' ); ?></label>
					<br />
					<input style="width: 100%" type="text" name="ywot_tracking_code" id="ywot_tracking_code"
						placeholder="<?php esc_attr_e( 'Enter tracking code', 'yith-woocommerce-order-tracking' ); ?>"
						value="<?php echo esc_attr( $order_tracking_code ); ?>" />
				</p>

				<p>
					<label
						for="ywot_carrier_name"> <?php esc_html_e( 'Carrier name:', 'yith-woocommerce-order-tracking' ); ?></label>
					<br />
					<input style="width: 100%" type="text" id="ywot_carrier_name" name="ywot_carrier_name"
						placeholder="<?php esc_attr_e( 'Enter carrier name', 'yith-woocommerce-order-tracking' ); ?>"
						value="<?php echo esc_attr( $order_carrier_name ); ?>" />
				</p>

				<p class="form-field form-field-wide">
					<label
						for="ywot_pick_up_date"> <?php esc_html_e( 'Pickup date:', 'yith-woocommerce-order-tracking' ); ?></label>
					<br />
					<input style="width: 100%" type="text" class="date-picker-field" id="ywot_pick_up_date"
						name="ywot_pick_up_date"
						placeholder="<?php esc_html_e( 'Enter pick up date', 'yith-woocommerce-order-tracking' ); ?>"
						value="<?php echo esc_attr( $order_pick_up_date ); ?>"
						pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
				</p>

				<p>
					<label><input type="checkbox"
								name="ywot_picked_up" <?php echo esc_attr( $order_picked_up ); ?> ><?php esc_html_e( 'Order picked up', 'yith-woocommerce-order-tracking' ); ?>
					</label>
				</p>
			</div>
			<?php
		...
		public function save_order_tracking_metabox( $post_id ) {
			...
			if ( $order ) {
				...
				if ( isset( $_POST['ywot_picked_up'] ) ) {
					$parameters['ywot_picked_up'] = sanitize_text_field( wp_unslash( $_POST['ywot_picked_up'] ) );
				}
