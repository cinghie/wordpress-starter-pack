...
if ( ! class_exists ( 'YITH_Shipping' ) ) {
	...
	class YITH_Shipping extends YITH_Document {
		...
		private function init_document() {
			$this->exists = yit_get_prop ( $this->order, '_ywpi_has_shipping_list', true );
			
			if ( $this->exists ) {
				$this->save_path = yit_get_prop ( $this->order, '_ywpi_shipping_list_path', true );
			}
		}
		...
		public function reset() {
			yit_delete_prop ( $this->order, '_ywpi_has_shipping_list' );
			yit_delete_prop ( $this->order, '_ywpi_shipping_list_path' );
		}
		...
		public function save() {
			...
			yit_save_prop ( $this->order,
				array(
					'_ywpi_has_shipping_list'  => $this->exists,
					'_ywpi_shipping_list_path' => $this->save_path
				) );
