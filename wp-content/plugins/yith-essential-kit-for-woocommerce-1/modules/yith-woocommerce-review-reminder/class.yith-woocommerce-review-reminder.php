	...
	class YWRR_Review_Reminder {
		...
		public function __construct() {
			...
			if ( is_admin() ) {
				...
				add_action( 'wp_enqueue_scripts', array( $this, 'ywrr_frontend_scripts' ) );
		 ...
		 * @author  Alberto Ruggiero
		 */
		public function ywrr_frontend_scripts() {
			global $post;

			if ( ! $post->ID == get_option( 'ywrr_unsubscribe_page_id' ) ) {
				return;
			}

			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

			wp_enqueue_script( 'ywrr-unsubscribe', YWRR_ASSETS_URL . 'js/ywrr-unsubscribe' . $suffix . '.js' );

			$params = array(
				'ajax_url' => add_query_arg( 'action', 'ywrr_unsubscribe', str_replace( array( 'https:', 'http:' ), '', admin_url( 'admin-ajax.php' ) ) ),
			);

			wp_localize_script( 'ywrr-unsubscribe', 'ywrr', $params );
		...
		public function ywrr_unsubscribe() {
			echo '<div class ="woocommerce ywrr-unsubscribe-form">';
